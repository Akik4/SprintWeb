...
-- Story 1
SELECT nom, date FROM animaux
INNER JOIN historique 
ON animaux.id = historique.id
WHERE date < "2003-01-01";

-- Story 2
SELECT animaux.nom, espece, enclos.nom AS nom_enclos FROM animaux 
INNER JOIN enclos_animaux
...
-- Story 3
SELECT animaux.nom, espece, enclos.niveau AS niveau_enclos FROM animaux 
INNER JOIN enclos_animaux
ON animaux.id = enclos_animaux.id_animal
INNER JOIN enclos
ON enclos_animaux.id_enclot = enclos.id
WHERE espece = "Orang-Outan" OR espece = "lion";

-- Story 4
SELECT nom, taille, espece, date FROM animaux
INNER JOIN historique 
ON animaux.id = historique.id
WHERE espece = "Elephant" AND date < "2003-01-01" AND taille > 130;

-- Story 5
SELECT enclos.nom, niveau, COUNT(animaux.nom) AS nombre_animaux FROM enclos
INNER JOIN enclos_animaux
...
-- Story 6
SELECT nom, date, espece FROM animaux
INNER JOIN historique 
ON animaux.id = historique.id
WHERE espece = "Faucon"
ORDER BY date DESC;
...
-- Story 7
SELECT nom, date, espece, TIMESTAMPDIFF(MONTH, date, NOW()) AS age FROM animaux
INNER JOIN historique 
ON animaux.id = historique.id
WHERE espece = "Chèvre" OR "Pinguin"
ORDER BY date DESC;

-- Story 8
SELECT espece as current_espece, nom, (
    select sum(taille) from animaux where espece = current_espece
    ) as poids FROM animaux
...
SELECT year(date_entree) as annee, (
    select count(id) from enclos_animaux
	where year(enclos_animaux.date_entree) = annee)
    from enclos_animaux
order by annee ASC

-- Story 11

select TIMESTAMPDIFF(year, date_entree, now()) as âge, animaux.sexe, animaux.nom, animaux.espece, enclos.nom from enclos_animaux
LEFT JOIN animaux
ON animaux.id = enclos_animaux.id_animal
LEFT JOIN enclos
ON enclos.id = enclos_animaux.id_enclot
...
select animaux.*, TIMESTAMPDIFF(year, enclos_animaux.date_entree, now()) from enclos_animaux 
LEFT JOIN animaux
ON animaux.id = enclos_animaux.id_animal
left join enclos
on enclos_animaux.id_enclot = enclos.id
